...
class Client:
    ...
    def __init__(self, host, port, buffer_size, receive_timeout, print_incoming_messages,
                 print_sent_messages, kernel, x_train, y_train, x_test, y_test,
                 learning_rate=None, lambda_parameter=None, gamma=None, iterations=None):
        """
        class constructor
        ...
        self.lambda_parameter = lambda_parameter if lambda_parameter is not None else 0.1
        self.iterations = iterations if iterations is not None else 10000
    ...
    def join_cycle(self):
        ...
        exchange_messages_thread = ExchangeMessagesThread(buffer_size=self.buffer_size,
                                                          ...
                                                          kernel=self.kernel, x_train=self.x_train,
                                                          y_train=self.y_train, x_test=self.x_test, y_test=self.y_test,
                                                          learning_rate=self.learning_rate,
                                                          lambda_parameter=self.lambda_parameter, gamma=self.gamma,
                                                          iterations=self.iterations, random_secret=self.random_secret)
...
class ExchangeMessagesThread(threading.Thread):
    ...
    def __init__(self, buffer_size, receive_timeout, this_socket, receive_start_time, initial_key,
                 print_incoming_messages, print_sent_messages, kernel, x_train, y_train, x_test, y_test,
                 learning_rate, lambda_parameter, gamma, iterations, random_secret):
        """
        class constructor
        :param buffer_size: size of exchanged message
        :param receive_timeout: time before client drops connection if no response from server
    ...
    def reply(self):
        ...
        # if the message is entitled as 'Model', the training shall start
            ...
            # choose the model based on the preferences in client_main
            if self.mode == "linear":
                model = svm_linear.SVM_Linear(x_train=self.x_train, y_train=self.y_train,
                                              x_test=self.x_test, y_test=self.y_test, learning_rate=self.learning_rate,
                                              lambda_param=self.lambda_parameter, iterations=self.iterations)
                if self.message_content != "First Model":
                    model.w = np.array(self.message_content["Weights"])
                    model.b = np.array(self.message_content["Bias"])
            elif self.mode == "hingeloss":
                model = svm_hingeLoss.SVM_RBF(x_train=self.x_train, y_train=self.y_train,
                                              x_test=self.x_test, y_test=self.y_test, learning_rate=self.learning_rate,
                                              lambda_param=self.lambda_parameter, gamma=self.gamma,
                                              iterations=self.iterations)
                if self.message_content != "First Model":
                    model.w = self.message_content["Weights"]
                    model.b = self.message_content["Bias"]
            ...
            self.response_content = {"samples_number": len(self.x_train), "weights": w, "bias": b}
            self.send()
    ...
    def send(self):
        ...
        if self.response_subject == "Connect":
            self.socket.sendall(pickle.dumps(self.response))
        else:
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    print("\n\tAn error occurred while reading the received message")
                    pass
