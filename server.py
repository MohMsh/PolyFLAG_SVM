...
class server:
    ...
    def __init__(self, host, port, backlog_queue, minimum_clients, timeout, buffer_size, table_size,
                 print_incoming_messages, print_sent_messages, aggregation_method, x_test, y_test, global_rounds):
        """
        class constructor
    ...
    def start(self):
        ...
        client_thread = listen_thread(server_socket=server_socket, minimum_clients=self.minimum_clients,
                                      ...
                                      x_test=self.x_test, y_test=self.y_test, global_rounds=self.global_rounds,
...
class listen_thread(threading.Thread):
    ...
    def __init__(self, server_socket, minimum_clients, buffer_size, timeout, print_incoming_messages,
                 print_sent_messages, aggregation_method, x_test, y_test, global_rounds, current_rounds, table_size):
        """
        class constructor
        ...
        if type(received_message) is dict:
            ...
            if (self.message_subject is not None) and (self.message_content is not None):
                ...
                if self.message_subject == "Connect":
                    ...
                    self.response = {"subject": self.response_subject, "content": "First Model"}
                    ...
                    """
                    try:
                        clients_samples = self.decrypted_message["content"]["samples_number"]
                        client_weights = self.decrypted_message["content"]["weights"]
                        client_bias = self.decrypted_message["content"]["bias"]

                        if clients_samples != 0 and client_weights is not None and client_bias is not None:

                            client_param_dict = {"client": self.client_socket,
                                                 "clients_samples": clients_samples,
                                                 "client_weights": client_weights,
                                                 "client_bias": client_bias}

                            clients_parameters.append(client_param_dict)
                        else:
                            print("\n\tThe received gradients are not feasible")
                    except BaseException as e:
                        print("\n\tError occurred: {msg}".format(msg=e))

                    if len(clients_parameters) >= self.minimum_clients:
                        print("\n\t/|\\ Parameters received from all clients /|\\")
                        model = global_model.GlobalModel(self.w_agg, self.b_agg, clients_parameters,
                                                         aggregation_method=self.aggregation_method,
                                                         x_test=self.x_test, y_test=self.y_test,
                                                         current_round=self.current_round)
                        self.w_agg, self.b_agg = model.aggregate()

                        self.response_subject = "Model"
                        self.response_content = {"Weights": self.w_agg, "Bias": self.b_agg}
